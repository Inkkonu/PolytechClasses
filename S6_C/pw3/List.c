...
{
  if (list->size == 0)
  { // If the list is empty, adding at the end is the same as adding at the beginning
...
SCell *AddElementAfter(SList *list, SCell *cell, Data elem)
{
  ...
  {
    SCell *currentCell = list->head;
    while (currentCell != NULL && currentCell != cell)
    { // Find the cell you passed in parameter in the list
      currentCell = currentCell->next;
    }
    if (currentCell == NULL)
    { // If the cell isn't in the list, you can't create a new cell
      return NULL;
    }
    if (currentCell->next == NULL)
    { // If the cell passed in parameter is the tail
...
void DeleteCell(SList *list, SCell *cell)
{
  if (cell != NULL && list->size != 0)
  {
    ...
    if (currentCell != NULL)
    { // If you found the cell passed in parameter
      if (currentCell->next == NULL)
      { // If the cell you want to delete is the tail
      ...
      else
      {
        ...
        { // If the cell you want to delete is the head
          currentCell->next->prev = NULL;
          list->head = currentCell->next;
          free(currentCell);
        }
        else
        { // If it's a "regular" cell
