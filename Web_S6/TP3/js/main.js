    ...
    fetch('http://localhost:3000/genres')
        ...
        .then(response => {
            ...
            select.addEventListener('change', () => {
                const select = document.querySelector('select');
                loadArtists(response.find(elem => elem['id'] === select.options[select.selectedIndex].value));
            });
...
async function artistSelected(evt) {
    const response = await fetch('http://127.0.0.1:3000/artists/' + evt.target.parentElement.id + '/albums');
    if (!response.ok) {
        throw new Error('Response not OK');
    }
    ...
    popup.style.top = `${(document.body.clientHeight / 2.0) - (popup.clientHeight / 2.0)}px`;
    popup.style.left = `${(document.body.clientWidth / 2.0) - (popup.clientWidth / 2.0)}px`;
