...
class HashMap:
    ...
    def get(self, k: str):
        i = 0
        l = len(self.arr)
        index = int(hash(k, l) + 0.5 * i + 0.5 * (i**2)) % l
        first_item = self.arr[index]
        while self.arr[index].key != k and self.arr[index] is not self.sentinelle:
            if (
                self.arr[index] is first_item and i != 0
            ):  # If it looped, the process stops
                break
            i += 1
            index = int(hash(k, l) + 0.5 * i + 0.5 * (i**2)) % l
        if self.arr[index] is self.sentinelle or (
            self.arr[index] is first_item and i != 0
        ):
    ...
    def put(self, k: str, v: int):
        ...
        while self.arr[index] is not self.sentinelle and self.arr[index] is not None:
            if (
                ...
                break
            if (
                self.arr[index].key == k
            ):  # If the user tries to add a key that already exists, it overwrites the value
        ...
        if self.arr[index] is first_item and i != 0:
            ...
            self.size += 1
        if (
            self.size >= self.max_size * 2 / 3
        ):  # If the hashmap is quite filled, it automatically creates a new one
